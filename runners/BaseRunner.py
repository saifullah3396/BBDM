...
class BaseRunner(ABC):
    def __init__(self, config):
        ...
        (
            ...
            self.config.result.sample_to_eval_path,
        ) = make_save_dirs(
            self.config.args,
        ...
        (
            ...
            self.optimizer,
            self.scheduler,
        ) = self.initialize_model_optimizer_scheduler(self.config)
        ...
        self.use_ema = (
            False
            if not self.config.model.__contains__("EMA")
            else self.config.model.EMA.use_ema
        )
        if self.use_ema:
    ...
    def load_model_from_checkpoint(self):
        ...
        if (
            ...
            print(
                f"load model {self.config.model.model_name} from {self.config.model.model_load_path}"
            )
            ...
            if self.config.args.train:
                if (
                    self.config.model.__contains__("optim_sche_load_path")
                    and self.config.model.optim_sche_load_path is not None
                ):
                    optimizer_scheduler_states = torch.load(
                        self.config.model.optim_sche_load_path, map_location="cpu"
                    )
                    for i in range(len(self.optimizer)):
                        self.optimizer[i].load_state_dict(
                            optimizer_scheduler_states["optimizer"][i]
                        )

                    if self.scheduler is not None:
                        for i in range(len(self.optimizer)):
                            self.scheduler[i].load_state_dict(
                                optimizer_scheduler_states["scheduler"][i]
                            )
        return model_states
    ...
    def sample_step(self, train_batch, val_batch):
        self.apply_ema()
        self.net.eval()
        sample_path = make_dir(
            os.path.join(self.config.result.image_path, str(self.global_step))
        )
        if self.config.training.use_DDP:
    ...
    def train(self):
        ...
        try:
            ...
            for epoch in range(start_epoch, self.config.training.n_epochs):
                ...
                for train_batch in pbar:
                    ...
                    if (
                        ...
                        % (self.update_ema_interval * accumulate_grad_batches)
                        == 0
                    ):
                        self.step_ema()
                    ...
                    with torch.no_grad():
                        ...
                        if (
                            self.global_step
                            % int(self.config.training.sample_interval * epoch_length)
                            == 0
                        ):
                            # val_batch = next(iter(val_loader))
                            ...
                            if (
                                not self.config.training.use_DDP
                                or (
                                    self.config.training.use_DDP
                                    and self.config.training.local_rank
                                )
                                == 0
                            ):
                                val_batch = next(iter(val_loader))
                                self.sample_step(
                                    val_batch=val_batch, train_batch=train_batch
                                )
                ...
                print(
                    "training time: " + str(datetime.timedelta(seconds=elapsed_rounded))
                )
                ...
                if (
                    ...
                    or self.global_step > self.config.training.n_steps
                ):
                    if (
                        not self.config.training.use_DDP
                        or (
                        ...
                        with torch.no_grad():
                            ...
                            (
                                model_states,
                                optimizer_scheduler_states,
                            ) = self.get_checkpoint_states(stage="epoch_end")
                            ...
                            while temp < epoch + 1:
                                remove_file(
                                    os.path.join(
                                        self.config.result.ckpt_path,
                                        f"latest_model_{temp}.pth",
                                    )
                                )
                                remove_file(
                                    os.path.join(
                                        self.config.result.ckpt_path,
                                        f"latest_optim_sche_{temp}.pth",
                                    )
                                )
                                temp += 1
                            torch.save(
                            ...
                            torch.save(
                                model_states,
                                os.path.join(
                                    self.config.result.ckpt_path, f"last_model.pth"
                                ),
                            )
                            torch.save(
                                optimizer_scheduler_states,
                                os.path.join(
                                    self.config.result.ckpt_path, f"last_optim_sche.pth"
                                ),
                            )
                            ...
                            optim_sche_ckpt_name = (
                                f"top_optim_sche_epoch_{epoch + 1}.pth"
                            )
                            ...
                            if self.config.args.save_top:
                                ...
                                if top_key not in self.topk_checkpoints:
                                    ...
                                    if (
                                        average_loss
                                        < self.topk_checkpoints[top_key]["loss"]
                                    ):
                                        print(
                                            "remove "
                                            + self.topk_checkpoints[top_key][
            ...
            if (
                not self.config.training.use_DDP
                or (self.config.training.use_DDP and self.config.training.local_rank)
                == 0
            ):
                print("exception save model start....")
                print(self.__class__.__name__)
                model_states, optimizer_scheduler_states = self.get_checkpoint_states(
                    stage="exception"
                )
                torch.save(
                    model_states,
