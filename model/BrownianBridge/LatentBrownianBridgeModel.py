...
class LatentBrownianBridgeModel(BrownianBridgeModel):
    def __init__(self, model_config):
        ...
        # vae = AutoencoderKL.from_pretrained(na me, subfolder='vae')

        name = "stabilityai/sdxl-vae"
        self.vqgan = AutoencoderKL.from_pretrained(name).eval()
    ...
    def forward(self, x, x_cond, context=None):
        ...
        # plt.show()
        # plt.imshow(x_cond[0].permute(1, 2, 0).detach().cpu())
        # plt.show()
        
        # plt.imshow(self.decode(x_latent, cond=False)[0].permute(1, 2, 0).detach().cpu())
        # plt.show()

        # plt.imshow(
        #     self.decode(x_cond_latent, cond=True)[0].permute(1, 2, 0).detach().cpu()
        # )
        # plt.show()
